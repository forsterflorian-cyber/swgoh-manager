  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-gray-400">Loading upgrade advisory…</div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-gray-300">
              One recommendation model for public board and member workspace. Guild currently sits at{' '}
              <span className="font-semibold text-white">
                {data.summary.currentGuildCoverage}%
              </span>
              . With these upgrades the guild could reach{' '}
          ...
          <div className="flex items-center gap-4">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-400">slots possible</div>
            </div>
            <Button variant="secondary" onClick={handleExportReport}>
              {exported ? '✓ Copied!' : '📋 Report for Discord'}
            </Button>
          </div>
        </div>
      </Card>
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-4">
          <div>
            <label className="text-sm text-gray-400 block mb-1">Member</label>
            <select
            ...
            >
              <option value="all">Alle Member</option>
          ...
          <div>
            <label className="text-sm text-gray-400 block mb-1">Phase & category</label>
            <select
              value={phaseCategoryFilter}
              onChange={(e) => setPhaseCategoryFilter(e.target.value)}
              className="rounded-xl border border-gray-800 bg-gray-950 px-3 py-2 text-sm text-white outline-none"
            >
              <option value="all">All incomplete scopes</option>
        ...
        {filteredMembers.length === 0 ? (
          <Card>
            <p className="text-center text-gray-400 py-8">
              No upgrade recommendations match the current filters.
            </p>
          </Card>
        ) : (
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    Code: {member.allyCode} · Current: {member.currentContributions} slots
                  </p>
                </div>
                <Badge variant={member.potentialGain > 5 ? 'success' : member.potentialGain > 2 ? 'warning' : 'neutral'}>
                  +{member.potentialGain} slots possible
                </Badge>
              </div>
